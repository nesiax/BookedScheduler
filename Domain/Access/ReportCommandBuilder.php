...
class ReportCommandBuilder
{
    ...
    "rs"."title" as "title", "rs"."status_id" as "status_id",
        "ri"."reference_number", "ri"."start_date", "ri"."end_date", "ri"."checkin_date", "ri"."checkout_date", "ri"."previous_end_date", "ri"."credit_count", TIMESTAMPDIFF(\'second\'::text, "ri"."start_date"::timestamp, "ri"."end_date"::timestamp) as "duration",
                            ...
                            (SELECT ARRAY_TO_STRING(ARRAY_AGG(CONCAT("participant_users"."fname", \' \', "participant_users"."lname")), \'!sep!\')
                                FROM "reservation_users" "participants" INNER JOIN "users" "participant_users" ON "participant_users"."user_id" = "participants"."user_id" WHERE "participants"."reservation_instance_id" = "ri"."reservation_instance_id" AND "participants"."reservation_user_level" = 2 group by "participant_users"."user_id" ORDER BY "lname", "fname") as "participant_list",
                            (SELECT ARRAY_TO_STRING(ARRAY_AGG(CONCAT("cav"."custom_attribute_id",\'=\', "cav"."attribute_value")), \'!sep!\')
                            ...
                            (SELECT ARRAY_TO_STRING(ARRAY_AGG("g"."name"), \',\')
                                FROM "groups" "g" LEFT JOIN "user_groups" "ug" ON "g"."group_id" = "ug"."group_id" WHERE "ug"."user_id" = "owner"."user_id") as "user_group_list"
                                ';

    const COUNT_FRAGMENT = 'COUNT(1) as "total", TIMESTAMPDIFF(\'second\'::text, "ri"."start_date"::timestamp, "ri"."end_date"::timestamp) as "duration"';

    const TOTAL_TIME_FRAGMENT = 'SUM( extract (epoch from (LEAST("ri"."end_date", @endDate))::timestamp) - extract( epoch from (GREATEST("ri"."start_date", @startDate))::timestamp) ) AS "totalTime", TIMESTAMPDIFF(\'second\'::text, "ri"."start_date"::timestamp, "ri"."end_date"::timestamp) as "duration"';
    ...
    const ADD_GROUP_BY_DURATION_FRAGMENT = '"ri"."start_date", "ri"."end_date"';

    const ADD_GROUP_BY_DURATION_LIST_FRAGMENT = '"ri"."start_date", "ri"."end_date", "resources"."name", "resources"."resource_id", "schedules"."schedule_id", "schedules"."name"';

    const ADD_GROUP_BY_RESOURCE_LIST_FRAGMENT = '"resources"."name", "resources"."resource_id"';

    const ADD_GROUP_BY_ACCESSORY_LIST_FRAGMENT = '"accessories"."accessory_name", "accessories"."accessory_id", "ar"."quantity"';

    const ADD_GROUP_BY_GROUP_LIST_FRAGMENT = '"groups"."name", "groups"."group_id"';

    const ADD_GROUP_BY_USER_LIST_FRAGMENT = '"owner"."fname", "owner"."lname", "owner"."email", "owner"."user_id", "owner"."organization"';

    const ADD_GROUP_BY_SCHEDULE_LIST_FRAGMENT = '"schedules"."schedule_id", "schedules"."name"';
    ...
    private function GetGroupBy()
    {
        ...
        if ($this->fullList
            ...
            || ($this->count))
        {
            $groupBy->AppendGroupBy(self::ADD_GROUP_BY_DURATION_FRAGMENT);
        }

        if ($this->duration)
        {
            $groupBy->AppendGroupBy(self::ADD_GROUP_BY_DURATION_LIST_FRAGMENT);
        }

        if ($this->listResources && ($this->fullList || $this->groupByResource))
        {
            $groupBy->AppendGroupBy(self::ADD_GROUP_BY_RESOURCE_LIST_FRAGMENT);
        }

        if ($this->listAccessories && $this->fullList)
        {
            $groupBy->AppendGroupBy(self::ADD_GROUP_BY_ACCESSORY_LIST_FRAGMENT);
        }

        if ($this->listGroups)
        {
            $groupBy->AppendGroupBy(self::ADD_GROUP_BY_GROUP_LIST_FRAGMENT);
        }

        if ($this->listUsers)
        {
            $groupBy->AppendGroupBy(self::ADD_GROUP_BY_USER_LIST_FRAGMENT);
        }

        if ($this->listSchedules)
        {
            $groupBy->AppendGroupBy(self::ADD_GROUP_BY_SCHEDULE_LIST_FRAGMENT);
        }
    ...
    private function GetLimit()
    {
        ...
        if (!empty($this->limit))
        {
            $limit->Append("LIMIT 0 OFFSET {$this->limit}");
        }
...
class ReportQueryFragment
{
    ...
    public function AppendGroupBy($group_by_sql)
    {
        if (strlen($group_by_sql) > 0) {
